...
case "$1" in
  ...
  *)
    ...
    make clean
    make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j 5
    ...
    cp $base/busybox/config ./.config && make clean && make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j 5 && mv busybox ..
    ...
    make clean && make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- -j 5 PROGRAMS="dropbear dbclient dropbearkey dropbearconvert scp" MULTI=1 STATIC=1 strip && mv dropbearmulti $base/artifacts/dropbearmulti
