...
function modules_copy {
	...
	do
		...
		/bin/bash $base/hw4.sh ssh_cmd "cat > /lib/modules/4.11.0/$module" < $base/artifacts/$module 2> /dev/null
		echo -e "\e[31mManually check file integrity\e[0m"
		echo "**************************************"
		/bin/bash $base/hw4.sh ssh_cmd "md5sum /lib/modules/4.11.0/$module"
		md5sum $base/artifacts/$module
		echo "**************************************"
		cd $base/artifacts
	done
  cd $base
  $base/hw4.sh ssh_cmd "chmod +x /lib/modules/4.11.0/*.test"
	return
}
function modules_load {
	...
	do
		cd ..
		echo "Load $module to VM"
		/bin/bash $base/hw4.sh ssh_cmd "insmod /lib/modules/4.11.0/$module.ko"
		cd $base/artifacts
	done
	return
}
function modules_unload {
	...
	do
		...
		/bin/bash $base/hw4.sh ssh_cmd "rmmod $module"
		cd $base/artifacts
	done
	return
}
function modules_test {
	cd $base/artifacts
  list=$(ls *.ko| cut -d" " -f9 | cut -d"." -f1)
  cd ..
	for module in $list
	do
		echo "Test $module to VM"
		$base/hw4.sh ssh_cmd "/bin/bash -c /lib/modules/4.11.0/$module.ko.test"
	done
	return
}
